...
module Element : sig
  ...
  val null : t [@@js.custom let null = t_of_js Ojs.null]
  ...
  (* May return Element.null *)
  val last_child : t -> t

  (* May return Element.null *)
  val next_sibling : t (* T *) -> t
  ...
  val add_event_listener :
    t (* T *) -> string -> (Event.t -> unit) -> bool -> unit
...
module FileReader : sig
  type state = Empty [@js 0] | Loading [@js 1] | Done [@js 2] [@@js.enum]
...
module Canvas : sig
  ...
  val create_linear_gradient :
    context -> float -> float -> float -> float -> gradient
...
module Performance : sig
  val now : unit -> float [@@js.global "performance.now"]
end
...
module Svg : sig
  ...
  module PathElement : sig
    ...
    val create_moveto_rel : t -> float -> float -> PathSeg.t
      [@@js.call "createSVGPathSegMovetoRel"]

    val create_lineto_abs : t -> float -> float -> PathSeg.t
      [@@js.call "createSVGPathSegLinetoAbs"]

    val create_lineto_rel : t -> float -> float -> PathSeg.t
      [@@js.call "createSVGPathSegLinetoRel"]
  end
end
