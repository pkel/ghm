...
module Query = struct
  ...
  type _constr = {param: Request.Url.param lazy_t; inner: string lazy_t}
  ...
  let opn name left right args =
    let args =
      List.map ~f:(fun x -> force x.inner) args |> String.concat ~sep:","
    in
  ...
  module Tsvector = struct
    let websearch ?(regconfig="simple") =
      op Fn.id ("wfts(" ^ regconfig ^ ")")
  end
...
module Make (Request : REQUEST) = struct
  ...
  let conv_resp_list ~f =
    conv_resp ~f:(fun json ->
        Yojson.Safe.Util.convert_each f json |> Or_error.all)

  let select r = Url.param "select" (String.concat ~sep:"," r.select)

  let create r =
    request POST Url.(url r.name [select r])
    |> give_json |> map ~f:r.provide
  ...
  let read ?filter ?(order = []) ?limit ?offset r =
    ...
    let params =
      ...
      ; ( match order with
        | [] ->
            None
        | _ ->
            Some (param "order" (String.concat ~sep:"," order)) )
    ...
    |> want_json |> conv_resp_list ~f:r.return
  ...
  let delete filter r =
    request DELETE (Url.url r.name Query.[force filter.param])
end
